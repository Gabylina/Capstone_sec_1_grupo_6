...
export function ProcessModule5({ process, readOnly = false }: ProcessModule5Props) {
  ...
  const { user } = useAuth()
  /** La encuesta puede registrarse aunque el proceso esté finalizado o en solo lectura (admin). */
  const canEditEncuesta =
    user?.role === "admin" || (user?.role === "consultor" && !readOnly)
      ...
      if (response.success && response.data) {
        ...
        const contractedData = response.data
          ...
          .map((candidate: any) => ({
            ...
            satisfaction_survey_pending:
              candidate.contratacion_status === "contratado" && !candidate.encuesta_respondida,
            contratacion_status: candidate.contratacion_status,
            id_contratacion: candidate.id_contratacion ?? null,
            encuesta_respondida: candidate.encuesta_respondida ?? false,
          }))
