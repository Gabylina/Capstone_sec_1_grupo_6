/** Servicios sin encuesta de satisfacción */
...
export function getServiciosConEncuesta(): Array<{ codigo: string; modulo: number }> {
    return Object.entries(MODULO_FINAL_ENCUESTA).map(([codigo, modulo]) => ({ codigo, modulo }));
}
